...
Kazane は `v0.9 utsuwa / 器` Product Candidate の開発段階です。現在のmacOSアプリには、
...
v0.8の外部ベータ導線として、パートナー向けオンボーディング、Feedback
Context記録、Trust & Privacy説明、Chronicle Replayを実装済みです。残る完了
条件は、実際の外部試用者がフローと1件の由来を理解できることの確認です。

v0.9では、ローカルバックアップ／復元、利用者ロール、認可強制、初回起動時の
データ案内、診断出力まで実装しています。署名・notarizationとクリーン環境での
release rehearsalは未完了であり、公開用v0.9 candidateはまだありません。
