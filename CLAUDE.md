...
| **v0.8** | **hibiki / 響き** | **🔄 実装中（WI-801〜805 着手）** |

### ローカルデプロイ状態

- **バージョン:** `0.7.0`（コード更新済み、未ビルド）
- **インストール先:** `/Applications/Kazane.app`（現在 0.6.0 が稼働中）
- DB は `~/Library/Application Support/jp.zyxcorp.kazane/kazane.db` を引き継ぎ（migration v10 自動適用）
...
| WI | 内容 | 状態 |
|----|------|------|
| WI-801 | Agent Push通知 — 新WI割り当て時にエージェントへ通知 | inbox（未着手） |
| WI-802 | ADR-0013 Phase A — プロセス分離・権限境界の実装 | inbox（未着手） |
| WI-803 | Kazane MCP Server — scripts/kazane-mcp | ai（動作確認待ち） |
| WI-804 | エージェント間ルーティング — nextAgent フィールド | ai（E2Eテスト待ち） |
| WI-805 | 並列処理制御 — ロック機構 | ai（衝突テスト待ち） |
