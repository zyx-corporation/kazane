...
| **v0.9** | **utsuwa / 器** | **🔄 WI-901・902・903・905・909 Done、受入・配布準備中** |
...
kazane-agents.yaml    — エージェントランタイム構成（daemons/AI agent roster）
...
kazane-agent start                              # daemons 起動（kazane-agentd + kazaned）
kazane-agent stop                              # daemons 停止
kazane-agent ps                                # daemon状態 + AIエージェント一覧表示
```
※ DB操作系（create/update/sync-github-issues/report-test/status/read）はアプリを開いたままでも使用可。  
※ `kazane-import-dev` だけはアプリを閉じて実行すること。
※ `start/stop/ps` は `kazane-agents.yaml`（リポジトリルート）を読む。
