...
| WI | 内容 |
|----|------|
| WI-801 | Agent Push通知 — 新WI割り当て時にエージェントへ通知（現状ポーリングのみ） |
| WI-802 | ADR-0013 Phase A — kazaned / kazane-agentd プロセス分離・権限境界の実装 |
| WI-803 | Kazane MCP Server — 外部エージェントが `read_wi / submit_handoff` 等を呼べる口 |
| WI-804 | エージェント間ルーティング — WI を Agent A → Agent B へ渡すフロー |
| WI-805 | 並列処理制御 — 複数エージェントの同一WI取得衝突を防止 |
