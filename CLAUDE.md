...
**アプリバージョン:** `0.9.0`（v0.9開発中、DB migration v13 適用済み）
...
| v0.8 | hibiki / 響き | ✅ 完了（WI-801〜811 + WI-BAO-001〜003 Done、外部試用継続） |
| **v0.9** | **utsuwa / 器** | **🔄 WI-901・902 Done、P1実装中** |
...
| v13 | users（ロールモデル: owner/operator/reviewer/agent） |
...
| WI | 内容 | 状態 |
|----|------|------|
| WI-900 | v0.8外部試用ゲートを実観察で閉じる | 🔄 外部参加者待ち |
| WI-901 | backup/restoreと整合性検証 | ✅ done（round-trip 5テスト通過） |
| WI-902 | 利用者・ロールモデル | ✅ done（migration v13・ADR-0004更新・role fixtures） |
| WI-903 | ロール認可と監査の強制 | inbox（WI-902完了後に着手） |
| WI-904 | first-run setupとdata/recovery案内 | inbox |
| WI-905 | diagnosticsとsupport export bundle | inbox |
| WI-906 | Chronicle Replay hardening | inbox |
| WI-907 | 署名・notarization・candidate配布経路 | inbox |
| WI-908 | product・trust・support・pricing資料 | inbox |
| WI-909 | ADR-0014 Remote GUI境界の確定 | inbox |
| WI-910 | Product Candidate release rehearsal | inbox |

次の実装スライスは WI-903（ロール認可強制）。
...
| WI-BAO-001 | AI番頭: kazane-mcp 接続確認 | done（MCP 9ツール全疎通、Handoff永続化E2E済み） |
| WI-BAO-002 | AI番頭: Gmail→返信案→Handoff 自律実行 | done（講座申込メール WI-911 実運用E2E済み） |
| WI-BAO-003 | AI番頭: Handoff品質評価 | done（カバレッジ36%・エスカレ率0%・trust高97%確認済み） |
